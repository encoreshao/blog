...
> **Sitemap & RSS update automatically.** `/sitemap.xml` is generated by `@astrojs/sitemap` at build time and picks up all new pages. `/rss.xml` (EN) and `/zh/rss.xml` (ZH) are Astro API endpoints that query the content collections on every build — no manual step required.
